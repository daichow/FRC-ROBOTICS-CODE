...
public class TurretFixCommand extends Command {
    	...
    	if (Math.abs(RobotMap.centerX) > 5) {
    		if (slow) {
    			if (RobotMap.centerX > 0) {
		    		Robot.turretSubsystem.move(Robot.min((RobotMap.centerX) * 0.008, 0.05));
		    	} else if (RobotMap.centerX < 0){
		    		Robot.turretSubsystem.move(Robot.max((RobotMap.centerX) * 0.008, -0.05));
		    	}
    		} else {
    			if (RobotMap.centerX > 0) {
    ...
    protected boolean isFinished() {
    	if (slow) {
	    	return Math.abs(RobotMap.centerX) < 5;
	    } else {
	    	return Math.abs(RobotMap.centerX) < 5;
	    }
    }
