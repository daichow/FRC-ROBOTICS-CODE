...
public class AutoCommand extends CommandGroup {
    ...
    public void Auto2Command(int position, int speed){
        ...
        addSequential(new RotateCommand(0)); // back to 0 degrees
        addSequential(new DriveLidarCommand(40), 2.5);
		addSequential(new DriveLidarCommand(40));
        switch(position){ // drive forward and turn turret depending on position
	        case 2: addSequential(new TipperCommand(Direction.Reverse, Mode.Auto)); // tipper down
	        		addParallel(new TurretCommand(TurretPos.RC, Mode.Auto));
	        	    addSequential(new FlywheelCommand(Direction.Forward)); // flywheel on
	        	break;
	        case 3: addSequential(new TipperCommand(Direction.Reverse, Mode.Auto)); // tipper down
	        		addParallel(new TurretCommand(TurretPos.RC, Mode.Auto));
	        	    addSequential(new FlywheelCommand(Direction.Forward)); // flywheel on
	        	break;
	        case 4: addParallel(new TipperCommand(Direction.Reverse, Mode.Auto)); // tipper down
		    	    addSequential(new FlywheelCommand(Direction.Forward)); // flywheel on
	        	break;
	        case 5: addSequential(new TipperCommand(Direction.Reverse, Mode.Auto)); // tipper down
	        		addParallel(new TurretCommand(TurretPos.LC, Mode.Auto));
	        	    addSequential(new FlywheelCommand(Direction.Forward)); // flywheel on
	        	break;
