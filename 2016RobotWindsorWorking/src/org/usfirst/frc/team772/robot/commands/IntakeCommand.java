...
public class IntakeCommand extends Command {
    ...
    protected void initialize() {
    	...
    	if(mode != Mode.Variable){ // Auto or Manual
    		switch(direction){
	    		case Forward: Robot.intakeSubsystem.bottomIn();
	    			break;
	    		case Reverse: Robot.intakeSubsystem.out();
	    			break;
	    		case Stop: Robot.intakeSubsystem.move(0);
	    			break;
	    		default:
    		}
    	}
    ...
    protected boolean isFinished() {
    	if (mode == Mode.Auto) { // Auto
	    	if (Robot.ultrasonicSubsystem.isBallIn() || direction == Direction.Stop) { // returns when ball is in
	    		return true;
	    	} else {
	    		return isTimedOut();
	    	}
