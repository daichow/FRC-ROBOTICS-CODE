...
import org.usfirst.frc.team772.robot.commands.group.BallInCommand;
import org.usfirst.frc.team772.robot.commands.group.BallOutCommand;
import org.usfirst.frc.team772.robot.commands.group.IntakeInRumbleCommand;
...
//		j1button1.whenPressed(new FeedPickUpCommand()); // auto pick up
		j1button1.whenPressed(new IntakeCommand(Direction.Forward, Mode.Auto)); // auto intake ball
//		j1button1.whenPressed(new IntakeInRumbleCommand());
		j1button2.whileHeld(new TipperCommand(Direction.Reverse, Mode.Manual)); // tipper up manual
		j1button3.whileHeld(new TipperCommand(Direction.Forward, Mode.Manual)); // tipper down manual
		j1button4.whenPressed(new BallInCommand()); // ball intake - shooter with rumble
...
//		
		j2button1.whenPressed(new FlywheelCommand(Direction.Forward));
		j2button2.whenPressed(new FlywheelCommand(Direction.Stop)); // fly stop
		j2button3.whenPressed(new FlywheelCommand(Direction.Reverse)); // fly reverse
		j2button4.whenPressed(new ShootCommand()); // just shoot
		...
		j2button9.whenPressed(new TurretCommand(TurretSubsystem.CENTER, Mode.Auto));
...
//		j2button5.whileHeld(new TurretCommand(TurretSubsystem0.LEFT, Mode.Manual)); // manual turret CCW
//		j2button6.whileHeld(new TurretCommand(TurretSubsystem.RIGHT, Mode.Manual)); // manual turret CW
